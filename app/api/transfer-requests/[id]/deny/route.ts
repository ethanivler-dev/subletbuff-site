...
import { sendTransferRequestDeniedEmail } from '@/lib/email'
...
export async function POST(
  ...
  if (transferRequest.landlord_id !== user.id) {
    return NextResponse.json({ error: 'You are not authorized to deny this request.' }, { status: 403 })
  }
  ...
  if (updateError) {
    console.error('[transfer-requests/deny] Update error:', updateError)
  ...
  // Send denial email
  sendTransferRequestDeniedEmail(
    transferRequest.applicant_email,
    transferRequest.applicant_name,
    listingTitle
  ).catch((err) => console.error('[transfer-requests/deny] Email error:', err))
